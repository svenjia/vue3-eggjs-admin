...
// 找出用户自己有的router权限
export const markRouterHidden = (routes: AppRouteRecordRaw[], resources: Resource[]) => {
  const length = routes.length
  const menuRes = resources.filter((item) => item.type === 1)
  for (let index = 0; index < length; index++) {
    const route = routes[index]
    if (route?.meta?.noPermission) {
      continue
    }
    const hasPermission = menuRes.some((item) => item.name === route.name)
    // 在权限列表中查看是否有对应的name

    route.meta = Object.assign(route?.meta || {}, { hidden: !hasPermission })

    // 如果有子路由，则递归调用
    if (route.children) {
      markRouterHidden(route.children, menuRes)
    }
  }
}
